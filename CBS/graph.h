...
class Graph{
    ...
    std::unordered_map<int, std::vector<int>> edges; // Parent node: (0,0) Child nodes: (0,1) (1,0)

public:
    Graph(const std::string& map_location, const std::string& agent_location);
    Graph() = default;
    void initialise_nodes(json input_map_json);     //used to initialise the nodes of the edges
    ...
    int assign_id_to_node(const Node& x) const;
